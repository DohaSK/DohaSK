# Hi, I'm Doha Skouf 👋

**Data Science & AI Engineering Student @ ENSA Fès (2027)**

🔭 Currently building: ML pipelines, RAG chatbots, Big Data warehouses
🌱 Learning: MLOps, Deep Learning, Multi-agent Systems
📫 Reach me: skoufd@gmail.com | +212 649 054 178
📍 Based in: Rabat, Morocco
🔍 Open to: Stage PFA (2 mois) — Juillet-Août 2026

---

## 🚀 Featured Projects

| Project | Stack | Highlights |
|---------|-------|------------|
| **[Well-being Chatbot](https://github.com/DohaSK/ai-chatbot-well_being)** | Flask, Gemini, Whisper | RAG system, 3 input modes, sentiment analysis |
| **[Insurance ML Pipeline](https://github.com/DohaSK/Assurance-vehicule-)** | XGBoost, SHAP, Flask | F1=0.906, REST API, MLOps monitoring |
| **[SNCF Data Warehouse](https://github.com/DohaSK/SNCF-Data-Warehouse)** | Talend, MySQL, Power BI | 16M+ records, €2.19B revenue analyzed |
| **[Web Scraping](https://github.com/DohaSK/Web-Scrabing)** | BeautifulSoup, Selenium | 500+ products, automated ETL |

---

## 🛠️ Tech Stack

**Languages:** Python, SQL, Java, C++
**ML/AI:** Scikit-learn, XGBoost, LightGBM, Deep Learning, LLMs, RAG, SHAP
**Data Engineering:** Apache Spark, Hadoop, Kafka, Talend ETL
**BI:** Power BI, DAX, Pandas, NumPy
**DevOps:** Git, Docker, MLflow, Prometheus

---

## 📜 Certifications

- 🚀 NASA — Open Science Badge
- 🎓 Columbia University — Machine Learning 1
- 🐍 IBM — Data Science with Python
- 🤖 DataCamp — LLMs Concepts

---

## 🌐 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?logo=linkedin)](https://linkedin.com/in/doha-skouf)
[![GitHub](https://img.shields.io/badge/GitHub-black?logo=github)](https://github.com/DohaSK)
[![Email](https://img.shields.io/badge/Email-red?logo=gmail)](mailto:skoufd@gmail.com)



